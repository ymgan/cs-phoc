...
event <- phoc_header %>%
  mutate(
    ...
    locality = "",
    countryCode = "AQ",  # I assume that the records are 
    locationID = "",
    geodeticDatum = "EPSG:4326",
    samplingProtocol = 'Please refer to sampling method in metadata'  
  ) %>%
  rename(eventID = header_id) %>%
  # fields that cannot be mapped to Darwin Core
  select(-season_name, -census_days, -census_date_start, -census_date_end, -surveyed_san_telmo)  
